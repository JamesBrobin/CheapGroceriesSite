    ...
    if data.get("foods"):
        ...
        for nutrient in food.get("foodNutrients", []):
            if nutrient.get("nutrientName") == "Energy" and nutrient.get("unitName") == "KCAL":
                print("Name:", data["foods"][0]["description"])
                return (nutrient.get("value"), data["foods"][0]["description"])  # calories per 100g/serving
    return (None, None)

if __name__ == "__main__":
    food_name = "Kroger dried black beans"  # change this to anything you want
    result = get_calories_from_usda(food_name)
